...
var uniteRequire = function (node, currentModulePath) {
    var foreignModuleRealPath = path.resolve(path.dirname(currentModulePath), node.args[0].value);  //获取依赖模块的绝对地址
...
var writeUglifyCode = function (result, moduleId, mainModule) {
    combinedCode += result.print_to_string(generateOptions);
    compressedCode += getCompressedCode(result) + '\n';
...
var initConfigs = function (config) {
    var configHandle = {
        "string": function (_config) {
            var realPath = path.resolve(__dirname, _config);
            return require(realPath);
        },
        "object": function (_config) {
            return _config;
        }
    };

    if (configHandle[type(config)] === undefined) {
        throw "wrong config: " + config;
    }
    commonJSConfig = configHandle[type(config)](config);
};
