...
var writeUglifyCode = function (result, moduleId, mainModule) {
    ...
    combinedCode += moduleCode.replace(/(define)\((['"]?)([^'"\s]*)\2\s*,\s*\[[^\]]*\]\s*,\s*function\(/g, function (match, $1, $2) {
        return match.replace(/^define/g, 'define' + unicodeMd5);
    });
    ...
    compressedCode += getCompressedCode(uglify.parse(moduleCode))
            .replace(/(define)\((['"]?)([^'"\s]*)\2\s*,\s*\[[^\]]*\]\s*,\s*function\(/g, function (match, $1, $2) {
                return match.replace(/^define/g, 'define' + unicodeMd5);
            }) + '\n';
