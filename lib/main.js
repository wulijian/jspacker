...
var unicode, md5, unicodeMd5, defineMd5;

var setMd5 = function () {
    unicode = '' + Date.now();
    md5 = crypto.createHash('md5');
    unicodeMd5 = md5.update(unicode)
        .digest('hex')
        .substring(0, 6);
    defineMd5 = 'define' + unicodeMd5;
};
...
var uniteDefine = function (node, currentModulePath) {
    ...
    node.expression.name = defineMd5;
    return node;
};
...
var uniteRequire = function (node, currentModulePath) {
    var foreignModuleRealPath = resolvepath(path.dirname(currentModulePath), node.args[0].value, scriptBasePath);  //获取依赖模块的绝对地址
...
var handleModule = function (moduleRealPath) {
    ...
    return moduleAST.transform(new uglify.TreeTransformer(null, function (node, descend) {
        if (node instanceof uglify.AST_Call) {
            var nodeName = node.expression.name;
            if (nodeName === 'define' || nodeName === defineMd5) {
        ...
        if (node instanceof uglify.AST_Symbol && node.name === "define") {
            node.name = defineMd5;
            return node;
        }
    }));
...
exports.walkAllModules = function walkAll(modulePath, moduleId, pre, after) {
    try {
        var moduleRealPath = resolvepath(modulePath, moduleId, scriptBasePath);
        ...
        if (!!configs.extractTo) {
            var extractToBase = path.resolve(scriptBasePath, './' + configs.extractTo);
            var fileExtractTo = path.resolve(path.dirname(moduleRealPath).replace(scriptBasePath, extractToBase));
            var targetTemplateBase = path.join(extractToBase, './' + configs.paths.template);
            var sourceTemplateBase = path.join(scriptBasePath, './' + configs.paths.template);
            shell.mkdir('-p', targetTemplateBase);

            if (moduleRealPath.indexOf(path.join(scriptBasePath, './output')) === 0) {
                var templateName = path.basename(moduleRealPath, '.js');
                var targetTemplatePath = path.join(sourceTemplateBase, './' + templateName);
                shell.cp('-Rf', targetTemplatePath, targetTemplateBase);
                console.info('#yellow{[extract template]}', templateName);
            } else {
                shell.mkdir('-p', fileExtractTo);
                if (fs.existsSync(moduleRealPath)) {
                    shell.cp('-Rf', moduleRealPath, fileExtractTo);
                } else {
                    console.error('#red{[file not exist]}:' + moduleRealPath + '\n');
                }
                console.info('#yellow{[extract file]}: ', moduleRealPath.replace(scriptBasePath, ''));
            }
        } else {
            console.log(moduleRealPath.replace(scriptBasePath, ''));
        }
...
var initCommonJSModuleCore = function () {
    var moduleImp = fs.readFileSync(baseModuleFile + '.js', 'utf-8').toString()
        .replace(/window\.define/g, 'window.define' + unicodeMd5)
        .replace(/data-mainentry/g, 'data-mainentry_' + unicodeMd5);
    combinedCode += moduleImp;
...
var writeUglifyCode = function (result, moduleId, mainModule) {
    var moduleCode = result.print_to_string({
        "indent_start": 0,
        "indent_level": 4,
        "quote_keys": true,
        "space_colon": true,
...
exports.pack = function (mainModule, output, moduleName, rootPath, config) {
    ...
    compressedCode = ''; //合并并压缩的代码
    allModule = {};
    configs = resolvepath.initConfigs(config);
    scriptBasePath = path.resolve(__dirname, rootPath || process.cwd());//计算项目根路径，不存在，直接使用当前执行的位置
    ...
    console.info("#green{[concat files]}:");
    ...
    console.info("#green{[Compress done.]}");
    return unicodeMd5;
};